...
export default function BookingForm() {
  ...
  const isFullyBooked    = (dateStr) =>
    getSlotsForDate(dateStr).length === TIME_SLOTS.length;
  ...
  const isSlotBooked = (dateStr, slot) =>
    getSlotsForDate(dateStr).includes(slot);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log("Booking Submitted:", { date: selectedDate, time: selectedTime, playerName });

    setBookedSlots((prev) => {
      const currentSlots = prev[selectedDate] || [];
      return {
        ...prev,
        [selectedDate]: [...currentSlots, selectedTime],
      };
    });
            ...
            {selectedTime && (
              <div className="bg-slate-900 border border-slate-700/50 rounded-2xl p-6 shadow-2xl">
                ...
                <input
                  ...
                  autoComplete="name"
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-600 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/50 focus:border-amber-400/40 transition-all"
                />
                <button
                  type="submit"
